...
- **Puntuación del Comité Científico:** 93.41
...
a una máquina. Su tarea consiste en **encontrar ese cambio: el índice de carácter donde
...
lo anterior a este es humano; todo lo que se encuentra a partir de este ha sido generado por una máquina.
...
- El **`boundary_char_index`** es el desplazamiento de caracteres donde termina la Parte A:
...
| `dataset/test_public/`  | 380   | ✅ incluido (copia de desarrollo) | ejecutar su pipeline y autoevaluarlo localmente |

En el **momento de la evaluación**, su carpeta `dataset/test_public/` se **sustituye por un
conjunto de evaluación oculto**. Tiene el mismo formato, pero **sin `answers.jsonl`**. Su
...
Debe entregar **un único notebook, que debe llamarse `solution.ipynb`**. Se exige este nombre de archivo exacto. Cualquier otro será rechazado sin ejecutarse.
...
Para cada muestra, sea `p` el índice predicho y `t` el límite verdadero. La puntuación por muestra decrece exponencialmente con la distancia en caracteres:
...
- **Entorno:** una GPU (≈16 GB de VRAM), sin internet en el momento de la evaluación; el modelo
  permitido (indicado a continuación) ya está disponible. **Presupuesto de tiempo real: 10 minutos** para la
...
- **Modelo preentrenado permitido**: esta lista es exhaustiva; no se puede utilizar ningún otro peso preentrenado.
  ...
  - **bge-base-en-v1.5**: un **codificador** de texto de 110M parámetros (modelo de embeddings). Este
    produce embeddings de oraciones/pasajes; no es un modelo de lenguaje generativo. Puede
...
El `solution.ipynb` proporcionado es una referencia trivial: estima una única
«fracción media del límite» a partir de `dataset/train/` y predice esa misma fracción de
la longitud para cada pasaje de prueba. Obtiene una puntuación de **28.6** en la partición oculta
**test_leaderboard_a** y existe únicamente como plantilla ejecutable para el bucle
leer-`dataset/test_public/` → escribir-`answers.jsonl`.

La **puntuación del Comité Científico de 93.41**, medida en la misma partición y con el mismo
presupuesto de 10 minutos, procede de realizar fine-tuning del codificador permitido sobre `train` y localizar
el cambio como un punto de cambio entre oraciones. No es una cota superior: el máximo
