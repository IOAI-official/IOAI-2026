...
- **Pontuação do Comité Científico:** 93.41
...
- O **`boundary_char_index`** é o deslocamento em caracteres onde termina a Parte A:
  `text[:boundary_char_index]` é a parte humana e
  `text[boundary_char_index:].lstrip()` é a parte da máquina.
...
|--------|---------|------------------|-----------|
| `dataset/train/` | 1,221 | ✅ incluído | treinar / fazer o ajuste fino do seu método |
| `dataset/test_public/`  | 380   | ✅ incluído (cópia de desenvolvimento) | executar o seu pipeline e calcular localmente a sua própria pontuação |

No **momento da avaliação**, a sua pasta `dataset/test_public/` é **substituída por um conjunto
de avaliação oculto**. Tem o mesmo formato, mas **sem `answers.jsonl`**. O seu
...
`dataset/test_public/` local é uma estimativa razoável da sua pontuação na tabela classificativa.
...
- **Ambiente:** uma GPU (≈16 GB VRAM), sem Internet no momento da avaliação — o modelo
  ...
  execução completa — este tem de abranger qualquer treino / ajuste fino que efetue no momento da avaliação
...
O `solution.ipynb` fornecido é uma referência trivial: estima uma única
«fração média da fronteira» a partir de `dataset/train/` e prevê essa mesma fração do
comprimento para cada passagem de teste. Obtém **28.6** no conjunto oculto
**test_leaderboard_a** e existe apenas como um modelo executável para o ciclo
ler-`dataset/test_public/` → escrever-`answers.jsonl`.

A **pontuação de 93.41 do Comité Científico**, medida no mesmo conjunto e com o mesmo
limite de 10 minutos, resulta do ajuste fino do encoder permitido em `train` e da localização
da transição como um ponto de mudança ao longo das frases. Não é um limite superior — o máximo
nesta métrica é 100.
