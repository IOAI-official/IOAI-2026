...
- **Score du Comité scientifique :** 93.41
...
- **Modèle préentraîné autorisé** — cette liste est exhaustive ; aucun autre poids préentraîné
  ...
  - **bge-base-en-v1.5** — un **encodeur** de texte de 110M paramètres (modèle d’embedding). Il
    ...
    pouvez l’utiliser **tel quel (caractéristiques gelées) ou l’affiner sur la partition `train`**
...
Le `solution.ipynb` fourni est une référence élémentaire : il estime une unique
« fraction moyenne de frontière » à partir de `dataset/train/` et prédit cette même fraction de
la longueur pour chaque passage de test. Il obtient un score de **28.6** sur la partition cachée
**test_leaderboard_a** et sert uniquement de modèle exécutable pour la boucle
lire-`dataset/test_public/` → écrire-`answers.jsonl`.

Le **score de 93.41 du Comité scientifique**, mesuré sur la même partition et avec le même
budget de 10 minutes, est obtenu en affinant l’encodeur autorisé sur `train` et en localisant
le changement comme un point de rupture entre les phrases. Ce n’est pas une borne supérieure — le maximum
pour cette métrique est 100.
