...
- **Scorul Comitetului Științific:** 93.41
...
Fișierul `solution.ipynb` furnizat este o referință trivială: estimează o singură
„fracție medie a graniței” din `dataset/train/` și prezice aceeași fracție din
lungime pentru fiecare pasaj de test. Acesta obține scorul **28.6** pe subsetul ascuns
**test_leaderboard_a** și există doar ca șablon executabil pentru bucla
citire-`dataset/test_public/` → scriere-`answers.jsonl`.

**Scorul Comitetului Științific de 93.41**, măsurat pe același subset și cu același
buget de 10 minute, provine din ajustarea fină a encoderului permis pe `train` și localizarea
trecerii drept punct de schimbare la nivelul propozițiilor. Acesta nu este o limită superioară — valoarea maximă
a acestei metrici este 100.
