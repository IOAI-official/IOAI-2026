...
- **Pontuação do Comitê Científico:** 93.41
...
- O **`boundary_char_index`** é o deslocamento de caracteres em que a Parte A termina:
...
- **Ambiente:** uma GPU (≈16 GB de VRAM), sem internet no momento da avaliação — o modelo
  permitido (abaixo) já é fornecido. **Orçamento de tempo de relógio: 10 minutos** para a
...
O `solution.ipynb` fornecido é uma referência trivial: ele estima uma única
“fração média da fronteira” a partir de `dataset/train/` e prevê essa mesma fração do
comprimento para cada passagem de teste. Ele obtém **28.6** no conjunto oculto
**test_leaderboard_a** e existe apenas como um modelo executável para o fluxo
ler-`dataset/test_public/` → escrever-`answers.jsonl`.

A **pontuação do Comitê Científico de 93.41**, medida no mesmo conjunto e com o mesmo
orçamento de 10 minutos, é obtida fazendo fine-tuning do encoder permitido em `train` e localizando
a mudança como um ponto de mudança entre sentenças. Ela não é um limite superior — o máximo
nessa métrica é 100.
