...
- **Baseline-score:** 28.6
- **Score van het Wetenschappelijk Comité:** 93.41
...
Er gebeuren vreemde dingen in het Nationaal Archief van Kazachstan. Bibliothecarissen zeggen dat sommige boeken vroeger anders eindigden, maar niemand kan het bewijzen — elk exemplaar is hetzelfde en elk verhaal is nog steeds logisch. Je bent als AI-onderzoeker uitgenodigd om de wijzigingen te vinden.
![Spook](../../ghost.jpg)

Een passage begint als door een mens geschreven tekst en schakelt op een bepaald moment ongemerkt over
...
De meegeleverde `solution.ipynb` is een triviale referentie: deze schat één
„gemiddelde grensfractie” op basis van `dataset/train/` en voorspelt voor
elke testpassage diezelfde fractie van de lengte. Deze behaalt **28.6** op de verborgen
**test_leaderboard_a**-split en bestaat uitsluitend als een uitvoerbaar sjabloon voor de
lees-`dataset/test_public/` → schrijf-`answers.jsonl`-lus.

De **score van het Wetenschappelijk Comité van 93.41**, gemeten op dezelfde split en binnen hetzelfde
budget van 10 minuten, is behaald door de toegestane encoder op `train` te finetunen en
de overgang als een veranderpunt over zinnen te lokaliseren. Dit is geen bovengrens — het maximum
voor deze metriek is 100.
