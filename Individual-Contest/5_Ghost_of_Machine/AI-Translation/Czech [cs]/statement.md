...
Poskytnutý `solution.ipynb` je triviální referenční řešení: odhadne jediný
„průměrný podíl hranice“ z `dataset/train/` a pro každou testovací pasáž předpoví stejný podíl
její délky. Na skryté části **test_leaderboard_a** dosahuje skóre **28.6**
a slouží pouze jako spustitelná šablona pro postup
načíst-`dataset/test_public/` → zapsat-`answers.jsonl`.

**Skóre vědecké komise 93.41**, naměřené na stejné části a se stejným
limitem 10 minut, pochází z doladění povoleného enkodéru na `train` a lokalizace
přechodu jako bodu změny mezi větami. Nejde o horní mez — maximum
této metriky je 100.
