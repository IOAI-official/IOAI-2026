...
- **Điểm baseline:** 28.6
- **Điểm của Ủy ban Khoa học:** 93.41
...
- **`boundary_char_index`** là độ lệch ký tự tại nơi Phần A kết thúc:
  `text[:boundary_char_index]` là phần do con người viết và
  `text[boundary_char_index:].lstrip()` là phần do máy viết.
...
tập đánh giá ẩn**. Tập này có cùng định dạng nhưng **không có `answers.jsonl`**. Notebook của bạn
được chạy lại trên tập này, và `answers.jsonl` mà notebook tạo ra sẽ được chấm điểm.
...
Cả ba tập đánh giá
đều có cùng kích thước và được lấy từ cùng phân phối với `train`, vì vậy điểm
...
- `dataset/train/` (có đáp án) luôn khả dụng khi bạn huấn luyện hoặc fine-tune.
...
- **≈0.78** — lệch 25 ký tự; - **≈0.61** — lệch 50 ký tự;
- **≈0.37** — lệch 100 ký tự;
- **≈0.01** — lệch 500 ký tự.
...
- **Môi trường:** một GPU (≈16 GB VRAM), không có internet tại thời điểm chấm — mô hình được phép
  (bên dưới) đã được cung cấp sẵn. **Ngân sách thời gian thực: 10 phút** cho
  toàn bộ lần chạy — khoảng thời gian này phải bao gồm mọi quá trình huấn luyện / fine-tune mà bạn thực hiện tại thời điểm chấm
  **cộng với** suy luận trên tập đánh giá.
- **Mô hình pretrained được phép** — danh sách này là đầy đủ; không được sử dụng bất kỳ trọng số pretrained nào khác.
  Mô hình này được **cung cấp sẵn trong môi trường** (hãy tải theo cách thông thường, ví dụ:
  `from_pretrained`; không có internet tại thời điểm chấm):
  - **bge-base-en-v1.5** — một **encoder** văn bản có 110M tham số (mô hình embedding). Mô hình này
    tạo ra embedding của câu/đoạn văn; nó không phải là mô hình ngôn ngữ sinh. Bạn
    có thể sử dụng nó **nguyên trạng (các đặc trưng đóng băng) hoặc fine-tune nó trên split `train`**
    (fine-tune toàn bộ phù hợp với ngân sách 16 GB / 10 phút).
- Các công cụ cổ điển / thống kê không bị hạn chế: bạn có thể xây dựng bất kỳ mô hình dựa trên đặc trưng nào
  (ví dụ: các bộ phân loại hoặc hồi quy scikit-learn) trên các đặc trưng embedding do chính bạn
  tính toán. *Các trọng số deep learning pretrained* chỉ bị giới hạn theo danh sách bên trên.

## Baseline

`solution.ipynb` được cung cấp là một tham chiếu đơn giản: nó ước lượng một
"tỷ lệ ranh giới trung bình" duy nhất từ `dataset/train/` và dự đoán cùng tỷ lệ đó trên
độ dài của mọi đoạn văn kiểm thử. Nó đạt **28.6** điểm trên split
**test_leaderboard_a** ẩn và chỉ tồn tại như một mẫu có thể chạy được cho quy trình
đọc-`dataset/test_public/` → ghi-`answers.jsonl`.

**Điểm 93.41 của Ủy ban Khoa học**, được đo trên cùng split và cùng
ngân sách 10 phút, đạt được bằng cách fine-tune encoder được phép trên `train` và xác định
điểm chuyển dưới dạng một changepoint trên các câu. Đây không phải là cận trên — điểm tối đa
của metric này là 100.
