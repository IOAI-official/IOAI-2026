...
à une machine. Votre tâche consiste à **trouver ce changement : l’indice du caractère où la partie générée par la machine commence, la partie écrite par un humain ayant terminé précédemment.**.

Chaque échantillon est une unique chaîne de caractères `text`. Il existe exactement une frontière (boundary en anglais, la première caractère du changement). Tout ce qui
la précède est humain ; tout ce qui se trouve à partir de celle-ci est généré par une machine.

## Dataset

Passages en anglais au format texte brut (plain text), avec une frontière chacun.
...
- Chaque partie contient au moins 180 mots ; la longueur totale est entre ~500–800 mots.
- Le **`boundary_char_index`** est le décalage en caractères où la Partie A se termine :
...
|--------|---------|------------------|-----------|
| `dataset/train/` | 1,221 | ✅ inclus | entraîner / affiner (fine-tune) votre méthode |
...
ensemble d’évaluation caché (hidden evaluation set)**. Il a le même format, mais **sans `answers.jsonl` (answers = réponses)**. Votre
notebook est réexécuté sur celui-ci, et le `answers.jsonl` qu’il produit est évalué.

- Le classement public utilise un ensemble caché (hidden set) **test_leaderboard_a** (380 échantillons).

- Le classement final utilise un ensemble caché (hidden set) **test_leaderboard_b** (380 échantillons).
...
one JSON object per line = un objet JSON par ligne; boundary = frontière; dev copy only ABSENT in the hidden grading set = uniquement copie en dev, absente dans l'ensemble d'évaluation caché
...
- `dataset/train/` (avec les réponses) est disponible chaque fois que vous entraînez ou affinez (fine-tune) un modèle.
...
- **Environnement :** un GPU (≈16 GB VRAM), sans internet au moment de l’évaluation — le modèle autorisé
  ...
  l’exécution complète — cela doit couvrir tout entraînement / affinage (fine-tuning) que vous effectuez au moment de l’évaluation,
...
- **Modèle préentraîné autorisé** — cette liste est exhaustive ; aucun autre poids préentraîné
  ...
  - **bge-base-en-v1.5** — un **encodeur** de texte de 110M paramètres (modèle d’embedding). Il
    ...
    pouvez l’utiliser **tel quel (as-is)(caractéristiques gelées (frozen features)) ou l’affiner (fine-tune) sur la partition `train`**
    (l’affinage complet respecte le budget de 16 GB / 10 minutes).
- Les outils classiques / statistiques ne sont soumis à aucune restriction : vous pouvez construire n’importe quel modèle
  fondé sur des caractéristiques (feature-based model) (p. ex., des classifieurs ou régresseurs scikit-learn) à partir de caractéristiques d’embedding (embedding features) que vous
  calculez vous-même. *Les poids de deep learning préentraînés (Pretrained deep-learning weights)* sont uniquement limités par la liste ci-dessus.
