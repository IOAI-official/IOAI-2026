...
**`solution.ipynb` という名前の、単一のnotebook**を提出します。この正確なファイル名が必須です。それ以外のものは実行されずに却下されます。
...
**最終スコアは、split(データセット)内の全サンプルに対するサンプルごとのスコアの平均**です（0–100 scaleで報告されます）。このmetricでは、完全一致だけでなく、境界に*近い*予測も評価されます。
