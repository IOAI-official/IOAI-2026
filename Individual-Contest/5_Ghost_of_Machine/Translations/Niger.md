...
## Output (format de soumission)

Vous devez soumettre **un notebook unique, qui doit être nommé `solution.ipynb`**. Ce nom de fichier exact est obligatoire. Tout autre fichier est rejeté sans être exécuté.
...
- Chaque identifiant dans `dataset/test_public/data.jsonl` doit apparaître exactement une fois. Un échantillon absent
  de `answers.jsonl` (ou associé à une valeur non entière / hors limites) donne un score de 0
