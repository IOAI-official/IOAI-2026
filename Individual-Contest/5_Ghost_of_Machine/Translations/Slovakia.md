...
- **Časť B** (od hranice ďalej): pokračovanie vytvorené jazykovým modelom, vytvorené na základe časťi A.
- Každá časť má aspoň 180 slov; celková dĺžka je ~500–800 slov.
- **`boundary_char_index`** je index **prvého znaku časti B**:
  `text[:boundary_char_index]` je ľudská časť spolu s jedinou medzerou oddeľujúcou obe časti a
  `text[boundary_char_index:]` je strojová časť.
...
- Každé ID v `dataset/test_public/data.jsonl` by sa malo objaviť práve raz. Vzorka, ktorá chýba
