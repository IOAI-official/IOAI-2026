...
- **Środowisko:** jedna GPU (≈16 GB VRAM), bez dostępu do internetu
- **Rozmiar rozwiązania:** `solution.ipynb` ≤ 1 MB
- **Wynik bazowy:** 0 
- **Wynik Komitetu Naukowego:** 96.99 
...
Podczas oceniania Twój folder `dataset/test_public/` zostanie w sposób niewidoczny zastąpiony dwoma ukrytymi zbiorami obrazów (`test_leaderboard_a` i `test_leaderboard_b`) używanymi do oficjalnego oceniania. Każdy z nich zawiera **100 obrazów** w formacie PNG oraz plik etykiet. 
...
- Każdy plik zawiera pojedynczy tensor zapisany za pomocą `torch.save`. Jego kształt musi wynosić`3 x H x W`, gdzie `H` i `W` odpowiadają **oryginalnej** rozdzielczości tego obrazu (nie `224 x 224`).
...
Zeszyt ostrzeże Cię o wszelkich problemach z formatem danych wyjściowych.

## 4. Ograniczenia

- **Modele:** Musisz użyć `torchvision.models.resnet18(pretrained=True)` i `timm.create_model('vit_tiny_patch16_224', pretrained=True)`. Żadne inne wytrenowane modele nie są dozwolone.
- **Potok transformacji (ang. transform pipeline; wymuszany podczas ewaluacji):** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. See Section 3 of `baseline.ipynb` w celu uzyskania szczegółów. 
- **Rozdzielczość zaburzenia:** Musi odpowiadać **oryginalnej** rozdzielczości surowego obrazu (nie 224×224). Tensor jest
  dodawany do surowego obrazu *przed* zastosowaniem potoku transformacji.
- **Format danych wyjściowych:** wyłącznie pliki `.pt` — bez PNG/JPG . Tensory są dodawane do surowego obrazu, a przed przetwarzaniem wstępnym wartości pikseli są ograniczane do `[0, 1]`.
...
PF jest funkcją zaprojektowaną tak, aby karać zaburzenia o wysokiej normie i być bardzo czułą w pobliżu górnej granicy wyników. Ona ona jest ograniczona do zakresu od 0.5 do 1. Pełną implementację można zobaczyć w Sekcji  8 pliku `solution.ipynb`. 
...
W Sekcji 7 zeszytu `solution.ipynb` znajdują się testy, które ostrzegą Cię o problemach z formatowaniem.
