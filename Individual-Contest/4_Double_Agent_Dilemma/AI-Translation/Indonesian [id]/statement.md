...
Pada saat penilaian, folder `dataset/test_public/` Anda secara transparan digantikan oleh dua himpunan gambar tersembunyi (`test_leaderboard_a` dan `test_leaderboard_b`) untuk penilaian resmi. Masing-masing berisi **100 gambar** dalam format PNG dan sebuah berkas label. 
...
`solution.ipynb` berisi contoh lengkap yang berfungsi. Berkas ini memuat data publik, kedua model, dan penilai (scorer) resmi, serta menulis berkas ZIP pengumpulan. Bacalah sebelum Anda mulai.
