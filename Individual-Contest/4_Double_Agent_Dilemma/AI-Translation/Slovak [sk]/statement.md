...
- **Časový limit:** 12 minutes.
- **Úložisko:** 5 GB
- **Prostredie:** jeden GPU (≈16 GB VRAM), bez internetu
- **Veľkosť riešenia:** `solution.ipynb` ≤ 1 MB
- **Baseline skóre:** 0 
- **Skóre vedeckého výboru:** 96.99 
...
- **Transformačný pipeline (vynútený pri vyhodnocovaní):** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. See Section 3 of `baseline.ipynb` obsahuje podrobnosti. 
...
- **Výstupný formát:** Iba súbory `.pt` — žiadne PNG/JPG . Tenzory sa pridajú k nespracovanému obrázku a hodnoty pixelov sa pred predspracovaním orežú na `[0, 1]`.
- **Názvy súborov:** Uloženie naplocho, striktný formát `{index}_a.pt` / `{index}_b.pt`. V súbore zip nesmú byť žiadne podpriečinky.
...
PF je funkcia navrhnutá tak, aby penalizovala perturbácie s vysokou normou a bola veľmi citlivá v blízkosti maxima výkonu. Je je ohraničená na rozsah 0.5 až 1. Úplnú implementáciu možno nájsť v časti  8 súboru `solution.ipynb`. 
...
`solution.ipynb` obsahuje úplný a funkčný príklad. Načíta verejné dáta, oba modely a oficiálny hodnotiaci nástroj a zapíše súbor ZIP na odovzdanie. Pred začatím si ho prečítajte.
