...
- **輸出格式：** 僅可使用 `.pt` 檔案——不得使用 PNG/JPG。 Tensor 會被加入原始影像（一般化至 `[0,1]`），且像素值在預處理前會被裁切至 `[0, 1]`。
...
PF 是一個用於懲罰具有高範數 (high norm) 之擾動的函數，並且在效能上限附近非常敏感。它被限制在 0.5 到 1 的範圍內。PF的完整實作可見於 `solution.ipynb` 的第  8 節。 
...
notebook 中設有檢查機制，若存在格式問題便會向你發出警示；該檢查機制位於 `solution.ipynb` notebook 的第 7 節。
