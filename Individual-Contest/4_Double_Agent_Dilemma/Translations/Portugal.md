...
Cada perturbação tem de ser suficientemente *pequena* para que seja difícil de detetar. Perturbações menores obtêm pontuações mais elevadas (consulte a Secção 5). A perturbação é aplicada diretamente à imagem original no nível dos píxeis.

## 2. Dados públicos

É fornecido com a tarefa um conjunto de imagens, organizado em duas partições — `train` (100 imagens) e
`test_public` (100 imagens) — cada uma destas partições com imagens de resolução variável. Todas as imagens pertencem às 1000 classes do ImageNet-1K, e tanto o Modelo R como o Modelo V obtêm uma precisão de 100% em ambas as partições.
...
Durante a avaliação, a sua pasta `dataset/test_public/` é substituída de forma transparente por um de dois conjuntos ocultos de imagens (`test_leaderboard_a` e `test_leaderboard_b`) usadas para as calcular as pontuaçõe oficiais. Cada uma contém **100 imagens** em formato PNG e um ficheiro de etiquetas. 
...
- Cada ficheiro é apenas um único tensor guardado com `torch.save`. A sua forma tem de ser`3 x H x W`, em que `H` e `W` correspondem à resolução **original** dessa imagem (e não `224 x 224`).
...
- **Pipeline de transformações (imposto durante a avaliação):** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. See Section 3 of `baseline.ipynb` para obter mais detalhes. 
- **Resolução da perturbação:** Tem de corresponder à resolução **original** da imagem (não 224×224). O tensor é
