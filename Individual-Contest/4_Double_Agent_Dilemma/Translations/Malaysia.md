...
Di Pusat AI Kebangsaan di Astana, dua model komputer — Model R (sebuah ResNet-18) dan Model V (sebuah ViT-Tiny) — sedang menganalisis foto. Pada masa ini, kedua-dua model melakukan tugas mereka dengan sempurna, boleh mencapai ketepatan 100% dan bersetuju untuk label setiap setiap imej. Untuk menguji sejauh mana "otak" pintar mereka sebenarnya berbeza, saintis agung memberi anda satu cabaran: buat perubahan piksel (pixel) yang kecil dan hampir tidak kelihatan pada setiap foto supaya Model R dan Model V tidak bersetuju sama sekali.

![img](../dilemma.jpg)

## 1. Tugasan

Dua pengelas imej pralatih melihat imej yang sama. Pada imej yang disediakan dalam tugasan ini, kedua-duanya mencapai ketepatan 100%.
...
Notebook anda akan memberi amaran jika terdapat apa-apa isu dengan format hasilan.
...
Skor akhir dikira seperti berikut. Biar `M` ialah bilangan imej dalam pecahan (split) tersebut, $Score_A$ bilangan perturbation Jenis A yang berjaya, dan $Score_B$ bilangan perturbation Jenis B yang berjaya:
$$
S_{final} = \frac{Score_A + Score_B}{2M} \times PF
$$

PF ialah satu fungsi terpendam yang direka dengan tujuan (1) menghukum "perturbation" yang mempunyai "norm" tinggi dan (2) memberi perbezaan yang teliti di antara rangkuman markah pengelasan imej yang cemerlang. Ia terbatas dalam julat 0.5 hingga 1. Implementasi penuh adalah ditunjuk dalam Seksyen 8 `solution.ipynb`. 
...
Terdapat pemeriksaan dalam notebook yang memberi amaran kepada anda jika terdapat isu pemformatan, dalam Seksyen 7 dalam notebook `solution.ipynb`.

## 7. Ujian setempat

`solution.ipynb` mengandungi satu contoh yang lengkap dan berfungsi. Ia memuatkan data awam, kedua-dua model, dan pemberi skor rasmi, serta menulis satu fail ZIP penghantaran. Sila membaca notebook itu sebelum anda mula.
