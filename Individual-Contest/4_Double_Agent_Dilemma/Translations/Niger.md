...
- Le code ne doit produire qu’un seul fichier ZIP, `submission.zip`. Placez tous les fichiers `.pt` dans l’archive ZIP, sans dossier englobant ni sous-répertoire. 
...
- **Pipeline de transformation (imposé lors de l’évaluation) :** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. Voir Section 3 of `baseline.ipynb` pour plus de détails. 
