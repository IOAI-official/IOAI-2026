...
2つの学習済み画像分類器が同じ画像を見ます。この課題で提供される画像に対して、両分類器は正解率(accuracy)100%を達成します。
...
- **コードは、`submission.zip`という1つのZIPファイルだけ**を生成する必要があります。すべての`.pt`ファイルを、格納フォルダやサブディレクトリを設けず、ZIPアーカイブの最上位に配置してください。 
...
`solution.ipynb`という名前のファイルだけを提出してください。
