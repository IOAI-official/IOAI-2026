...
Les fichiers suivants sont fournis (maps each image index to its correct class = attribue chaque indice de l'image  à sa classe correcte) :
...
Lors de l’évaluation, votre dossier `dataset/test_public/` est remplacé de manière transparente par deux ensembles cachés d’images (hidden sets of images) (`test_leaderboard_a` et `test_leaderboard_b`) pour le calcul officiel du score. Chacun contient **100 images** au format PNG et un fichier d’étiquettes. 
...
- Chaque fichier est un tenseur unique enregistré avec `torch.save`. Sa forme (shape) doit être`3 x H x W`, où `H` et `W` correspondent à la résolution **originale** de cette image (et non à `224 x 224`).
...
- **Résolution de la perturbation :** elle doit correspondre à la résolution **originale** de l’image brute (et non à 224×224) (raw image resolution). Le tenseur est
  ajouté à l’image brute (raw image) *avant* le pipeline de transformation.
- **Format de sortie :** fichiers `.pt` uniquement — aucun PNG/JPG . Les tenseurs sont ajoutés à l’image brute (raw image) (normalisation à [0,1]) et, ensuite, les valeurs sont écrêtées dans  (clipped to)  `[0, 1]` avant le prétraitement (preprocessing).
- **Nommage des fichiers :** liste à plat sans hierarchie (flat-listed), au format strict `{index}_a.pt` / `{index}_b.pt`. Aucun sous-répertoire dans le fichier zip.
