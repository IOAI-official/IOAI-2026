...
```text
train/images/*.png         # 100 images au format PNG 
train/labels.json          # associe à chaque index de l'image sa vraie classe
test_public/images/*.png   # 100 images au format PNG 
test_public/labels.json    # associe à chaque index de l'image sa vraie classe
```
...
**Remarque : pour cette tâche, les labels des datasets de test sont accessibles.**
...
- **Pipeline de transformation (imposé lors de l’évaluation) :** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. Regardez la Section 3 de  `baseline.ipynb` pour plus de détails. 
...
- **Format de sortie :** fichiers `.pt` uniquement — aucun PNG/JPG . Les tenseurs sont ajoutés à l’image (normalisée
à [0,1]) et les valeurs des pixels sont ramenés dans `[0, 1]` avant le preprocessing.
