...
- **Modèle V** : `timm's vit_tiny_patch16_224` (un Transformer, ViT-Tiny).
...
- **Format de sortie :** fichiers `.pt` uniquement — *aucun PNG/JPG*. Les tenseurs sont ajoutés à l’image (normalisé à `[0,1]`) et les valeurs des pixels sont écrêtées (clippé) dans `[0, 1]` avant le prétraitement.
