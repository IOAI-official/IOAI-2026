...
En el centro nacional de IA de Astaná, dos modelos informáticos —el Modelo R (un ResNet-18) y el Modelo V (un ViT-Tiny)— están analizando fotografías. En este momento, ambos modelos realizan un trabajo perfecto: obtienen una exactitud del 100% y coinciden en todas y cada una de las imágenes. Para comprobar cuán diferentes son realmente sus «cerebros» inteligentes, el científico jefe le plantea un desafío: realizar cambios diminutos, casi invisibles, en los píxeles de cada fotografía para que el Modelo R y el Modelo V estén completamente en desacuerdo.
...
Durante la evaluación, su carpeta `dataset/test_public/` se reemplaza de forma transparente por dos conjuntos ocultos de imágenes (`test_leaderboard_a` y `test_leaderboard_b`) para la puntuación oficial. Cada uno contiene **100 imágenes** en formato PNG y un archivo de etiquetas. 
...
- El código debe generar únicamente un archivo ZIP, `submission.zip`. Coloque todos los archivos `.pt` en el nivel superior del archivo ZIP, sin ninguna carpeta contenedora ni subdirectorios. 
...
- **Pipeline de transformaciones (aplicado obligatoriamente durante la evaluación):** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. See Section 3 of `baseline.ipynb` para más detalles. 
...
- **Formato de salida:** Solo archivos `.pt`; no PNG/JPG . Los tensores se añaden a la imagen sin procesar (que tiene sus pixeles normalizados entre [0, 1]) y luego el valor de los pixeles se recorta al rango [0, 1] antes de realizar el procesamiento.
...
La puntuación final se calcula de la siguiente manera. Sea `M` el número de imágenes de la partición, $Score_A$ el número de perturbaciones de Tipo A exitosas, que lograron confundir al primer modelo, y $Score_B$ el número de perturbaciones de Tipo B exitosas:
$$
