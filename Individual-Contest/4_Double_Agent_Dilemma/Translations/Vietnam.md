...
- **Loại A**: sau khi cộng nhiễu, Model R vẫn phân loại ảnh chính xác, nhưng Model V phân loại ảnh sai.
- **Loại B**: sau khi cộng nhiễu, Model V vẫn phân loại ảnh chính xác, nhưng Model R phân loại ảnh sai.
...
```text
train/images/*.png         # 100 ảnh ở định dạng PNG
train/labels.json          # ánh xạ mỗi chỉ mục ảnh tới nhãn đúng của nó
test_public/images/*.png   # 100 ảnh ở định dạng PNG
test_public/labels.json    # ánh xạ mỗi chỉ mục ảnh tới nhãn đúng của nó
```
...
```text
{index}_a.pt   # nhiễu loại A
{index}_b.pt   # nhiễu loại B
```
...
- **Độ phân giải của nhiễu:** Phải khớp với độ phân giải ảnh thô **gốc** (không phải 224×224). Tensor được cộng vào ảnh thô *trước* pipeline biến đổi.
- **Định dạng đầu ra:** Chỉ các tệp `.pt` — không có PNG/JPG . Các tensor được cộng vào ảnh thô (đã được chuẩn hóa về `[0, 1]`) và các giá trị sau đó được cắt về `[0, 1]` trước khi tiền xử lý.
