...
Votre tâche consiste à créer une petite modification (« perturbation » encore appele bruit ) pour chaque image afin que les deux modèles soient en désaccord. Pour chaque image, vous devez créer **deux perturbations différentes** :
...
Un ensemble d’images est fourni avec la tâche, organisé en deux splits (lots) `train` (100 images) et
...
- **Modèles :** vous devez utiliser le module de vision par ordinateur  de pytorch   `torchvision.models.resnet18(pretrained=True)` et `timm.create_model('vit_tiny_patch16_224', pretrained=True)`. **_Aucun autre modèle préentraîné n’est autorisé_**.
- **Pipeline de transformation (imposé lors de l’évaluation) :** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. voir la Section 3 de `baseline.ipynb` dans le notebook pour plus de détails. 
- **Résolution de la perturbation :** elle doit correspondre à la résolution **originale** de l’image brute qui est à verifier  minitieusement = (et non à 224×224). Le tenseur est ajouté à l’image brute *avant* le pipeline de transformation.
