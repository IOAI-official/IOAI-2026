...
Gjatë vlerësimit, dosja juaj `dataset/test_public/` zëvendësohet në mënyrë transparente nga dy bashkësi të fshehura imazhesh (`test_leaderboard_a` dhe `test_leaderboard_b`) për vlerësimin zyrtar. Secila prej tyre përmban **100 imazhe** në formatin PNG dhe një skedar etiketash. 

**Shënim: Për këtë detyrë, etiketat në dataset-et e testimit janë të qasshme.**

## 3. Formati i rezultatit
...
- Kodi duhet të prodhojë vetëm një skedar ZIP, `submission.zip`. Vendosini të gjithë skedarët `.pt` në nivelin më të lartë të arkivit ZIP, pa dosje përmbajtëse ose nëndosje. 
...
Notebook-u do t’ju njoftojë nëse ka ndonjë problem me formatin e rezultatit.
...
- **Pipeline-i i transformimit (zbatohet gjatë vlerësimit):** `Resize(256) → CenterCrop(224) →
  Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])`. Shikoni pjesën 3 të `baseline.ipynb` për detaje. 
...
- **Formati i rezultatit:** Vetëm skedarë `.pt` — jo PNG/JPG . Tensorët i shtohen imazhit të papërpunuar dhe vlerat e pikselëve kufizohen në `[0, 1]` përpara parapërpunimit.
- **Emërtimi i skedarëve:** Flat-list, me format të rreptë `{index}_a.pt` / `{index}_b.pt`. Pa nëndosje brenda skedarit zip.
- **Bibliotekat:** `torch`, `torchvision`, `timm`. 
...
![imazh](../curves.jpeg)
Figura: Kurba e funksionit të penalizimit.
...
`solution.ipynb` përmban një shembull të plotë dhe funksional. Ai ngarkon të dhënat publike, për dy modelet dhe vlerësuesin zyrtar, si dhe shkruan një skedar ZIP për dorëzim. Lexojeni përpara se të filloni.

## 8. Si të dorëzoni

- Ruani ndryshimet tuaja në `solution.ipynb`.
- Hapni skedarin Git në shiritin anësor në majtë të JupyterLab.
