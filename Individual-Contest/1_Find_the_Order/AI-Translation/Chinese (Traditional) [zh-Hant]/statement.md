...
你會獲得兩位參與者 *Speaker A* 與 *Speaker B* 之間的英語口語對話。每段對話會依說話者的發言輪次分段，每個輪次僅包含一位說話者的語音。每個輪次均儲存為一個獨立的 `.wav` 音訊檔案，因此一段完整對話由一組 `.wav` 檔案表示，每個輪次各有一個檔案。 
...
每段對話包含名為 `n`、`chunk_0.wav`、`chunk_1.wav`、…、`chunk_{n-1}.wav` 的音訊檔案。這些片段是個別的發言輪次。檔名僅對應打亂後的順序，並不表示片段在原始對話中的位置。每段對話有 7–20 個片段，單聲道，44.1 kHz（你可以
重新取樣）。

**`prefix.json` 包含每段對話前兩個片段的檔名索引。** 這會識別對話真正的開頭，並消除以正向或反向閱讀對話時所產生的歧義。
