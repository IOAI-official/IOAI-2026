...
Notebook của bạn được thực thi lại trên dữ liệu đó, và tệp `answers.json` mà nó tạo ra được dùng để chấm điểm. Các đoạn hội thoại kiểm thử được giữ lại đến từ cùng một phân phối với `train`, vì vậy điểm `test_public` cục bộ của bạn là một bản xem trước đáng tin cậy.
