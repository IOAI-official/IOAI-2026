...
Anda diberikan dialog lisan berbahasa Inggris antara dua peserta, *Speaker A* dan *Speaker B*. Setiap dialog disegmentasi menjadi giliran bicara (turn), dengan setiap giliran hanya berisi ucapan dari satu pembicara. Setiap giliran disimpan sebagai berkas audio `.wav` terpisah, sehingga satu dialog utuh direpresentasikan oleh sekumpulan berkas `.wav`, satu untuk setiap giliran.

Sayangnya, giliran-giliran tersebut telah diacak secara acak, sehingga percakapannya tidak lagi masuk akal. Pada nama berkas `chunk_{k}.wav`, `k` merujuk pada chunk ke-k dalam kumpulan yang telah diacak, bukan giliran ke-k dalam dialog aslinya.
...
Setiap dialog berisi `n` berkas audio bernama `chunk_0.wav`, `chunk_1.wav`, …, `chunk_{n-1}.wav`. Chunk merupakan giliran bicara individual. Nama berkas hanya berkaitan dengan urutan acak tersebut. Nama berkas tidak menunjukkan posisi suatu chunk dalam percakapan aslinya. Setiap dialog memiliki 7–20 chunk, mono, 44.1 kHz (Anda boleh melakukan resampling).

**`prefix.json` berisi indeks nama berkas dari dua chunk pertama pada setiap dialog.** Hal ini menandai awal sebenarnya dari dialog dan menghilangkan ambiguitas antara membaca percakapan maju atau mundur.

Sebagai contoh: `11: [7, 12]` berarti bahwa giliran pertama dan kedua dari dialog 11 masing-masing adalah `chunk_7.wav` dan `chunk_12.wav`.
...
Pada saat penilaian, folder `dataset/test_public/` Anda secara transparan digantikan oleh
sebuah `hidden evaluation set` (`test_leaderboard_a` untuk leaderboard publik dan `test_leaderboard_b` untuk leaderboard final) — folder-folder ini memiliki ukuran dan format yang sama dengan `dataset/test_public/` tetapi tanpa `answers.json`.

Notebook Anda dijalankan kembali pada data tersebut, dan berkas `answers.json` yang dihasilkannya digunakan untuk penilaian. Dialog uji yang disembunyikan berasal dari distribusi yang sama dengan `train`, sehingga skor `test_public` lokal Anda merupakan pratinjau yang akurat.
...
Untuk setiap dialog, tentukan urutan kronologis asli dari chunk audionya. Prediksi Anda harus berupa permutasi `P` dari `{0, 1, …, n−1}`, dengan `P[i]` adalah posisi kronologis prediksi dari `chunk_i.wav` (0 = pertama).
...
Urutan sebenarnya adalah **chunk_1 → chunk_2 → chunk_0**, sehingga `P = [2, 0, 1]`, dan `prefix.json` berisi `[1, 2]`.
...
- **Representasi ucapan:** **wav2vec 2.0**. **Whisper encoder** juga boleh digunakan sebagai ekstraktor fitur.
[wav2vec model card](https://yastatic.net/s3/contest/ioai/1/01_facebook_wav2vec2-base-960h_MODEL_CARD.html)

- **Pengenalan ucapan otomatis (ASR):** **OpenAI Whisper** (ukuran apa pun).
