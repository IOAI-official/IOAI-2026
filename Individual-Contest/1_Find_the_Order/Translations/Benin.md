...
Malheureusement, les tours ont été mélangés aléatoirement, de sorte que la conversation n’a plus de sens. Dans le nom de fichier `chunk_{k}.wav`, `k` désigne le k-ième segment (ou _chunk_) de l’ensemble mélangé, et non le k-ième tour du dialogue d’origine.
...
Chaque dialogue contient `n` fichiers audio nommés `chunk_0.wav`, `chunk_1.wav`, …, `chunk_{n-1}.wav`. Les segments (ou _chunk_) sont des tours de parole individuels. Les noms de fichiers correspondent uniquement à l’ordre mélangé. Ils n’indiquent pas la place d’un segment (ou _chunk_) dans la conversation d’origine. Chaque dialogue comporte 7–20 segments (ou _chunk_), en mono, à 44.1 kHz (vous pouvez ré-échantillonner).

**`prefix.json` contient les indices des noms de fichiers des deux premiers segments (ou _chunk_) de chaque dialogue.** Cela identifie le véritable début du dialogue et élimine l’ambiguïté entre la lecture de la conversation dans le sens chronologique ou dans le sens inverse.
...
| Dossier | Dialogues | `answers.json` ? | Utiliser le pour |
|--------|-----------|-----------------|-----------|
| `dataset/train/` | 1,288 | ✅ inclus | entraîner / affiner (_fine-tune_) votre modèle |
| `dataset/test_public/`  | 100   | ✅ inclus | exécuter votre pipeline et calculer vous-même votre score en local |

Lors de l’évaluation, votre dossier `dataset/test_public/` est remplacé de manière transparente par
un `hidden evaluation set` (`test_leaderboard_a` pour le classement public et `test_leaderboard_b` pour le classement final) — ceux-ci ont la même taille et le même format que `dataset/test_public/`, mais sans le fichier `answers.json`.
...
Pour chaque dialogue, déterminez l’ordre chronologique d’origine de ses segments audio (ou _chunk_). Votre prédiction doit être une permutation `P` de `{0, 1, …, n−1}`, où `P[i]` est la position chronologique prédite de `chunk_i.wav` (0 = premier).
...
Un dialogue comporte 3 segments (ou _chunk_) mélangés `chunk_0, chunk_1, chunk_2` :

| segment (ou _chunk_) mélangé | contenu parlé | position réelle (rang) |
|----------------|----------------|----------------------|
...
La métrique d’évaluation de cette tâche est **l’exactitude de l’ordre par paires**. Elle examine chaque paire de segments (ou _chunk_) et pose la question : _lequel des deux doit venir en premier ?_ Une paire est correcte si votre prédiction donne la même réponse que la vérité terrain. Pour un dialogue comportant `n` segments (ou _chunk_), il existe $$M = n(n-1)/2$$ paires ; soit `I` le nombre d’inversions — les paires ordonnées différemment de la vérité terrain :

$$\text{score} = 1 - \frac{I}{M} \in [0, 1]$$

ℹ️ **Le score final est la moyenne des scores par dialogue sur l’ensemble des
dialogues du split (la base de test par exemple).**
...
Notez que la limite de 10 minutes doit couvrir toute la phase d’entraînement ou de _fine-tuning_ (affinement) que vous effectuez au moment de l’évaluation, ainsi que l’inférence sur la base de test.
...
- Revenez sur cette page du _challenge_ et cliquez sur **Submit**.
