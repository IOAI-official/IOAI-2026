...
你會獲得兩位參與者 *Speaker A* 與 *Speaker B* 之間的英語口語對話。每段對話會根據說話者的發言輪次進行分段，每個輪次僅包含一位說話者的語音。每個輪次均被儲存為一個獨立的 `.wav` 音訊檔案，因此一段完整對話將由一組 `.wav` 檔案表示，其中一個檔案對應一個對話輪次。

不幸的是，這些輪次已被隨機打亂，因此對話不再連貫。在檔名 `chunk_{k}.wav` 中，`k` 指的是打亂後集合中的第 k 個片段，而非原始對話中的第 k 個輪次。
...
每段對話包含 `n` 個音訊檔案、分別被命名為 `chunk_0.wav`、`chunk_1.wav`、…、`chunk_{n-1}.wav`。這些片段是各自單獨的發言輪次。檔名僅對應打亂後的順序，並不表示片段在原始對話中的位置。每段對話有 7–20 個片段，單聲道，44.1 kHz（你可以
重新取樣）。

在 **`prefix.json` 中，包含每段對話前兩個片段的檔名索引。** 這表示對話真正的開頭，並消除以正向或反向閱讀對話時所產生的歧義。
...
| `dataset/test_public/`  | 100   | ✅ 包含 | 執行你的 pipeline 並在本地自行評分 |

在評分時，你的 `dataset/test_public/` 資料夾會被透明地替換為
`hidden evaluation set`（在公開排行榜中會使用 `test_leaderboard_a`，最終排行榜中會使用 `test_leaderboard_b`）——它們的大小與格式均和 `dataset/test_public/` 相同，但不含 `answers.json`。

你的 notebook 會在該資料集上再次執行，其產生的 `answers.json` 檔案將用於評分。留出測試對話與 `train` 測試集來自相同的分布，因此你在本機取得的 `test_public` 分數能如實預示結果。
...
對於每段對話，需要判定其音訊片段原本的時間順序。你的預測應為 `{0, 1, …, n−1}` 組成的一個排列 `P`，其中 `P[i]` 表示 `chunk_i.wav` 的預測時間位置（0 = 第一個）。
...
⚠️ **P 必須是真正的排列：** 長度為 n、以 0 做為起始索引，且每個值恰好出現一次。重複值、缺少值或超出範圍的項目（例如使用 1 起始索引），都會使該對話得到 0 分；檔案中缺少某段對話時亦同。格式錯誤或非 JSON 的檔案將被拒絕。
...
請注意，10 分鐘限制必須涵蓋你在評分時進行的任何訓練或微調，以及在評估集上的推理。
