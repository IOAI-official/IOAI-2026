...
Vous disposez de dialogues en anglais parlés entre deux participants, *Locuteur A* et *Locuteur B*. Chaque dialogue est segmenté (chunké) en tours de parole, chaque tour contenant la parole d’un seul locuteur. Chaque tour est stocké dans un fichier audio `.wav` distinct ; un dialogue complet est donc représenté par un ensemble de fichiers `.wav`, un pour chaque tour. 

Malheureusement, les tours ont été mélangés aléatoirement, de sorte que la conversation n’a plus de sens. Dans le nom de fichier `chunk_{k}.wav`, `k` désigne le k-ième segment (chunk) de l’ensemble mélangé, et non le k-ième tour du dialogue d’origine.
...
Chaque dialogue contient `n` fichiers audio nommés `chunk_0.wav`, `chunk_1.wav`, …, `chunk_{n-1}.wav`. Chacun de ces chunks est un tour de parole individuel. Les noms de fichiers correspondent uniquement à l’ordre mélangé. Ils n’indiquent pas la place d’un chunk dans la conversation d’origine. Chaque dialogue comporte 7–20 chunks, en mono, à 44.1 kHz (vous pouvez
rééchantillonner).

**`prefix.json` contient les indices des noms de fichiers des deux premiers chunks de chaque dialogue.** Cela identifie le véritable début du dialogue et élimine l’ambiguïté entre la lecture de la conversation dans le sens chronologique ou dans le sens inverse.
...
Vous recevez **deux dossiers au meme format** :

| Dossier | Dialogues | `answers.json` ? | A utiliser pour |
|--------|-----------|-----------------|-----------|
...
Pour chaque dialogue, déterminez l’ordre chronologique d’origine de ses chunks audio. Votre prédiction doit être une permutation `P` de `{0, 1, …, n−1}`, où `P[i]` est la position chronologique prédite de `chunk_i.wav` (0 = premier).
...
Un dialogue comporte 3 chunks mélangés `chunk_0, chunk_1, chunk_2` :

| chunk mélangé | contenu parlé | position réelle (rang) |
|----------------|----------------|----------------------|
...
La métrique d’évaluation de cette tâche est **l’exactitude de l’ordre par paires** (pairwise ordering accuracy). Elle examine chaque paire de chunks et pose la question : _lequel des deux doit venir en premier ?_ Une paire est correcte si votre prédiction donne la même réponse que la vérité terrain. Pour un dialogue comportant `n` chunks, il existe $$M = n(n-1)/2$$ paires ; soit `I` le nombre d’inversions — les paires ordonnées différemment de la vérité terrain :
...
- **Représentations de la parole :** **wav2vec 2.0**. L’**encodeur Whisper** peut également être utilisé comme extracteur de features.
...
- **Modèle de langage :** **Qwen2.5-0.5B**, qui peut être utilisé soit en zero-shot, soit fine-tuné sur le split `train` fourni.
[Fiche du modèle Qwen](https://yastatic.net/s3/contest/ioai/1/02_Qwen_Qwen2.5-0.5B_MODEL_CARD.html)
Notez que la limite de 10 minutes doit couvrir tout entraînement ou fine-tuning que vous effectuez au moment de l’évaluation (grade time), ainsi que l’inférence sur l’ensemble d’évaluation (evaluation set).

## Procédure de soumission

- Ouvrez `solution.ipynb` et exécutez toutes les cellules. Vérifiez qu’il écrit `answers.json` dans le répertoire de travail, avec une permutation pour chaque dialogue de `dataset/test_public/` (100 dialogues). Au moment de l’évaluation (grade time), le notebook est exécuté à nouveau sur l’ensemble de test caché (hidden test set), et le fichier `answers.json` qu’il y produit est évalué.
- Améliorez la solution si vous le souhaitez — ou ne le faites pas ; le baseline seule suffit à valider le pipeline.
- Ouvrez l’onglet Git dans la barre latérale gauche de JupyterLab.
- **Stage** `solution.ipynb` (l’icône + à côté du fichier).
