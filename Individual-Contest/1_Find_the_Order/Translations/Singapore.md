...
| `dataset/test_public/`  | 100   | ✅ 包含 | 运行你的流程并在本地自行评分 |

评分时，你的 `dataset/test_public/` 文件夹会被透明地替换为
...
你的 notebook 会在该数据上再次执行，其生成的 `answers.json` 文件将用于评分。留出的测试对话与 `train` 来自相同分布，因此你本地的 `test_public` 分数能够如实预示最终表现。
...
对于每段对话，确定其音频片段原始的时间顺序。你的预测应为 `{0, 1, …, n−1}` 的一个排列 `P`，其中 `P[i]` 是 `chunk_i.wav` 的预测时间位置（0 = 第一个）。
...
⚠️ **P 必须是一个真正的排列：** 长度为 n、从 0 开始索引，且每个值恰好出现一次。存在重复值、缺失值或越界项（例如从 1 开始索引）时，该对话得分为 0；文件中缺少某段对话时同样如此。格式错误或非 JSON 文件将被拒绝。

## 评分

本任务的评分指标为**成对顺序准确率**。它会检查每一对片段并询问：_两者中的哪一个应当在前？_ 如果你的预测与真实答案给出的结果相同，则该片段对正确。对于包含 `n` 个片段的对话，共有 $$M = n(n-1)/2$$ 个片段对；令 `I` 表示逆序数，即与真实答案顺序不同的片段对数量：
...
- **语言模型：** **Qwen2.5-0.5B**，既可以零样本使用，也可以在提供的 `train` 数据划分上进行微调。
