...
Két résztvevő, *A beszélő* és *B beszélő* közötti, angol nyelven elhangzó párbeszédeket kap. Minden párbeszéd (összefüggő) beszélői megszólalásokra van felosztva, és minden megszólalás csak egyetlen beszélő beszédét tartalmazza. Minden megszólalás külön `.wav` hangfájlban van tárolva, így egy teljes párbeszédet `.wav` fájlok halmazaként kapod meg, megszólalásonként egy fájllal. 
...
Minden párbeszéd `n` hangfájlt tartalmaz, amelyek neve `chunk_0.wav`, `chunk_1.wav`, …, `chunk_{n-1}.wav`. A hangrészletek egy-egy megszólalást tartalmaznak. A fájlnevek csak az összekevert sorrendnek felelnek meg, mely független attól, hogy egy hangrészlet hová tartozik az eredeti beszélgetésben. Minden párbeszéd 7–20 hangrészletből áll, monó, 44.1 kHz mintavételezési frekvenciával (újra-
mintavételezhető).
...
|--------|-----------|-----------------|-----------|
| `dataset/train/` | 1,288 | ✅ mellékelve | a modell tanítására / finomhangolására |
| `dataset/test_public/`  | 100   | ✅ mellékelve | a pipeline futtatására és helyi önértékelésre |

Az értékelés során az Ön `dataset/test_public/` mappáját lecserélik
...
A notebookot ezeken az adatokon ismét lefuttatják, és az általa létrehozott `answers.json` fájlt használják a pontozáshoz. A privát tesztpárbeszédek ugyanabból az eloszlásból származnak, mint a `train`, ezért az Ön helyi `test_public` pontszáma megbízható visszajelzést ad.

### Könyvtárstruktúra

```bash
dataset/train/
    prefix.json  # {dialogue_id: [first_idx, second_idx]} fájl indexe 
    answers.json  # {dialogue_id: P}  elvárt sorrend (sorrend konvenció)
    <dialogue_id>/
        chunk_0.wav
        ...
        chunk_{n-1}.wav
...
dataset/test_public/
    prefix.json
    answers.json     # kizárólag a fejlesztői változatban
    <dialogue_id>/
        chunk_0.wav
        ...
        chunk_{n-1}.wav
...
A `answers.json` kimeneti fájlnak minden párbeszéd-azonosítóhoz hozzá kell rendelnie az előre jelzett permutációt:
...
| összekevert hangrészlet | elhangzott tartalom | valódi pozíció (sorrend) |
|----------------|----------------|----------------------|
...
A feladat pontozásának mérőszáma a **páronkénti sorrendi pontosság** (pairwise ordering accuracy). Minden hangrészletpárt megvizsgál, és felteszi a kérdést: _a kettő közül melyiknek kell előbb következnie?_ Egy pár akkor helyes, ha az előrejelzés ugyanazt a választ adja, mint az elvárt. Egy `n` hangrészletből álló párbeszédben $$M = n(n-1)/2$$ pár van; legyen `I` az inverziók száma — azon pároké, amelyek sorrendje eltér az elvárttól:

$$\text{score} = 1 - \frac{I}{M} \in [0, 1]$$

ℹ️ **A végső pontszám az adathalmaz összes
...
- **Beszédreprezentációk:** **wav2vec 2.0**. A **Whisper encoder** jellemzőkinyerőként (feature
extractor) is használható.
...
- Nyissa meg a `solution.ipynb` fájlt, és futtassa az összes cellát. Ellenőrizze, hogy létrehozza-e a `answers.json` fájlt a munkakönyvtárban, a `dataset/test_public/` minden párbeszédéhez egy permutációt megadva (100 párbeszéd). Az értékeléskor a notebookot újra futtatják a rejtett teszthalmazon, és az ott létrehozott `answers.json` fájlt pontozzák.
- Ha szeretné, javítsa a megoldást — vagy ne tegye; a baseline önmagában lefuttat egy ellenőrzést a pipeline működésére.
- Nyissa meg a Git felületet a JupyterLab bal oldalsávjában.
...
- A push végrehajtásához (feltöltéshez) kattintson a felfelé mutató nyilat tartalmazó felhőikonra.
