...
Dane są mówione dialogi w języku angielskim między dwojgiem osób, *Mówcą A* i *Mówcą B*. Każdy dialog jest podzielony na wypowiedzi mówców (każda wypowiedź jest przypisna do konkretnego mówcy). Każda wypowiedź jest zapisana jako osobny plik audio `.wav`, zatem kompletny dialog jest reprezentowany przez zbiór plików `.wav`, po jednym dla każdej wypowiedzi. 
...
Każdy dialog zawiera `n` plików audio o nazwach `chunk_0.wav`, `chunk_1.wav`, …, `chunk_{n-1}.wav`. Pliki to pojedyncze wypowiedzi. Nazwy plików odpowiadają wyłącznie przetasowanej kolejności. Nie wskazują one, gdzie dany fragment należy umieścić w oryginalnej rozmowie. Każdy dialog ma 7–20 fragmentów, mono, 44.1 kHz (możesz zmienić częstotliwość próbkowania).
...
| `dataset/test_public/`  | 100   | ✅ dołączony | uruchomiania pipelinu i lokalnej oceny |

Podczas oceniania folder `dataset/test_public/` jest w sposób niewidoczny zastępowany przez
`hidden evaluation set` (`test_leaderboard_a` na potrzeby publicznej tabeli wyników oraz `test_leaderboard_b` na potrzeby końcowej tabeli wyników) — mają one taki sam rozmiar i format jak `dataset/test_public/`, ale bez `answers.json`.

Twój notebook jest ponownie wykonywany na tych danych, a utworzony przez niego plik `answers.json` jest używany do obliczenia wyniku. Wydzielone (ang. held-out) dialogi testowe pochodzą z tego samego rozkładu co `train`, więc lokalny wynik `test_public` stanowi wiarygodną zapowiedź wyniku.

### Struktura katalogów

```bash
dataset/train/
    prefix.json  # {dialogue_id: [first_idx, second_idx]} indeksy nazw plików 
    answers.json  # {dialogue_id: P}  złota kolejność
    <dialogue_id>/
        chunk_0.wav
        ...
        chunk_{n-1}.wav
...
dataset/test_public/
    prefix.json
    answers.json     # obecne tylko na "development copy"
    <dialogue_id>/
        chunk_0.wav
        ...
        chunk_{n-1}.wav
...
Twój plik wyjściowy `answers.json` powinien przypisywać każdemu identyfikatorowi dialogu (ang. dialogue ID) jego przewidywaną permutację:
...
⚠️ **P musi faktycznie być permutacją:** o długości n, indeksowaną od 0, zawierającą każdą wartość dokładnie raz. Duplikaty, brakujące wartości lub wartości spoza zakresu (np. indeksowane od 1) skutkują wynikiem 0 za dany dialog, podobnie jak brak dialogu w pliku. Plik o nieprawidłowym formacie lub plik niebędący plikiem JSON zostaje odrzucony.

## Ocenianie

Metryką oceny w tym zadaniu jest **trafność porządkowania parami (ang. pairwise ordering accuracy)**. Sprawdza ona każdą parę fragmentów i zadaje pytanie: _który z nich powinien wystąpić pierwszy?_ Para jest poprawna, jeśli twoja predykcja porządkuje ją tak samo jak złota kolejność (ang. ground truth). Dla dialogu zawierającego `n` fragmentów istnieje $$M = n(n-1)/2$$ par; niech `I` oznacza liczbę inwersji — par uporządkowanych inaczej niż w prawidłowym rozwiązaniu:

$$\text{score} = 1 - \frac{I}{M} \in [0, 1]$$

ℹ️ **Końcowy wynik jest średnią wyników dla poszczególnych dialogów w zbiorze.**

## Dozwolone modele

Do rozwiązania tego zadania możesz używać wyłącznie następujących modeli pretrenowanych, zarówno podczas trenowania, jak i ewaluacji. Wszystkie te modele są już pobrane i dostępne w środowisku. Przykłady ich użycia znajdziesz w bazowym notebooku `solution.ipynb`. Pamiętaj, że nie możesz używać żadnego innego modelu, a twój program nie ma dostępu do internetu.

- **Reprezentacje mowy:** **wav2vec 2.0**. **Enkoder modelu Whisper** może być również używany jako ekstraktor cech (ang. feature extractor).
...
- **Model językowy:** **Qwen2.5-0.5B**, którego można używać w trybie zero-shot albo dostroić na dostarczonym zbiorze `train`.
[Karta modelu Qwen](https://yastatic.net/s3/contest/ioai/1/02_Qwen_Qwen2.5-0.5B_MODEL_CARD.html)
Pamiętaj, że limit 10 minut musi obejmować zarówno trenowanie/dostrajanie które przeprowadzasz w czasie oceny rozwiązania, a także uruchomienie (inferencję) na zbiorze ewaluacyjnym.

## Jak przesłać rozwiązanie

- Otwórz `solution.ipynb` i uruchom wszystkie komórki. Potwierdź, że zapisuje on `answers.json` w katalogu roboczym, z permutacją dla każdego dialogu w `dataset/test_public/` (100 dialogów). Podczas oceniania notebook zostaje ponownie uruchomiony na ukrytym zbiorze testowym, a utworzony przez niego plik `answers.json` zostanie tam oceniony.
- Ulepsz rozwiązanie jeśli chcesz—albo tego nie rób, bo sam baseline wystarcza do sprawdzenia, że pipeline działa.
- Otwórz kartę Git na lewym pasku bocznym JupyterLab.
-  **Stage** `solution.ipynb` (ikona + obok niego).
- Wprowadź opis commita (ang. commit message) i kliknij **Commit**.
- Kliknij ikonę chmury ze strzałką w górę, aby zrobić push.
