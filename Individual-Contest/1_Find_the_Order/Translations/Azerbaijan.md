...
Təəssüf ki, növbələr təsadüfi olaraq qarışdırılıb, buna görə də dialoq mənasını itirmişdir. `chunk_{k}.wav` fayl adında `k` orijinal dialoqdakı k-cı növbəni deyil, qarışdırılmış dəstdəki k-cı fraqmenti bildirir.
...
Notebook-unuz həmin verilənlər üzərində yenidən icra olunur və onun istehsal etdiyi `answers.json` faylı qiymətləndirmə üçün istifadə olunur. Gizli saxlanılan sınaq dialoqları `train` ilə eyni paylanmadan gəlir, buna görə də lokal `test_public` xalınız dürüst bir önbaxışdır.
...
Həm öyrətmə, həm də qiymətləndirmə zamanı bu tapşırığı həll etmək üçün yalnız aşağıdakı əvvəlcədən öyrədilmiş modellərdən istifadə edə bilərsiniz. Bu modellərin hamısı artıq yüklənib və mühitdə mövcuddur. Onlardan necə istifadə olunacağına dair nümunələri `solution.ipynb` baseline notebook-da görə bilərsiniz. Nəzərə alın ki, başqa heç bir modeldən istifadə edə bilməzsiniz və proqramınızın internetə çıxışı yoxdur.

- **Nitq təmsilləri (representation):** **wav2vec 2.0**. **Whisper enkoderi** də xüsusiyyət çıxarıcısı (feature extractor) kimi istifadə oluna bilər.
...
- `solution.ipynb` faylını açın və bütün xanaları (cells) işlədin. İşçi qovluqda `dataset/test_public/`-dəki hər bir dialoq üçün (100 dialoq) permutasiyası olan `answers.json` faylının yazıldığını təsdiqləyin. Qiymətləndirmə zamanı notebook gizli test dəsti üzərində yenidən işlədilir və onun orada istehsal etdiyi `answers.json` qiymətləndirilir.
- İstəyirsinizsə həlli təkmilləşdirin — və ya etməyin; baseline təkcə pipeline doğrulayır.
- JupyterLab-ın sol yan panelində Git tab-ını açın.
...
`solution.ipynb` adında dəqiq bir fayl təqdim edin.
