...
Se proporcionan diálogos hablados en inglés entre dos participantes, *Persona A* y *Persona B*. Cada diálogo está segmentado en turnos de comunicacion, y cada turno contiene lo dicho por una unica persona. Cada turno se almacena como un archivo de audio `.wav` independiente, por lo que un diálogo completo está representado por un conjunto de archivos `.wav`, uno por cada turno. 
...
Cada diálogo contiene `n` archivos de audio denominados `chunk_0.wav`, `chunk_1.wav`, …, `chunk_{n-1}.wav`. Los fragmentos son turnos individuales. Los nombres de archivo corresponden únicamente al orden mezclado. No indican dónde corresponde un fragmento en la conversación original. Cada diálogo tiene 7–20 fragmentos, mono, 44.1 kHz (puedes
...
Durante la evaluación, tu carpeta `dataset/test_public/` se sustituye de forma transparente por
...
Tu notebook se ejecutara de nuevo con esos datos, y el archivo `answers.json` que genere se utiliza para calcular la puntuación. Los diálogos de prueba reservados proceden de la misma distribución que `train`, por lo que tu puntuación local de `test_public` es una estimación fiel.

### Estructura de directorios

```bash
dataset/train/
    prefix.json  # {dialogue_id: [primer_idx, segundo_idx]} indice del archivo 
    answers.json  # {dialogue_id: P}  orden real del dialogo (rank convention)
    {dialogue_id}/
        chunk_0.wav
        ...
        chunk_{n-1}.wav
...
Para cada diálogo, determina el orden cronológico original de sus fragmentos de audio. Tu predicción debe ser una permutación `P` de `{0, 1, …, n−1}`, donde `P[i]` es la posición cronológica predicha de `chunk_i.wav` (0 = primero).

El archivo de salida `answers.json` debe asociar cada ID de diálogo con su permutación predicha:
...
| fragmento mezclado | contenido hablado | posición verdadera (rank) |
|----------------|----------------|----------------------|
...
- **Modelo de lenguaje:** **Qwen2.5-0.5B**, que puede utilizarse tanto zero-shot como fine-tuneado con la partición `train` proporcionada.
[Ficha del modelo Qwen](https://yastatic.net/s3/contest/ioai/1/02_Qwen_Qwen2.5-0.5B_MODEL_CARD.html)
Ten en cuenta que el límite de 10 minutos debe abarcar cualquier entrenamiento o fine-tuning que se realice durante la evaluación, además de la inferencia sobre el conjunto de evaluación.
...
- Mejora la solución si deseas, o no lo hagas; el baseline por sí solo valida el pipeline.
...
- Haz click en el icono de la nube con una flecha hacia arriba para hacer push.
- Vuelve a esta página del concurso y haz clic en **Submit**.

Envia exactamente un archivo, denominado `solution.ipynb`.
