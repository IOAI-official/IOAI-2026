...
Ka gjashtë robotë. Çdo robot vepron në një dhomë të vogël të përfaqësuar nga një rrjetë. Çdo dhomë ka një zonë të luajtshme `6×6` të rrethuar me mure, kështu që vargu i plotë `image` ka madhësi `8×8` (zona e luajtshme + muret).
...
| `image` | dhoma, një varg numrash të plotë `8×8×2` ku kanali 0 përmban object_idx kategorik (p.sh., 1=bosh, 2=mur, 10=robot) dhe kanali 1 përmban colour_idx kategorik (0–5). |
...
`image[row][column] = [object_idx, colour_idx]`. Indeksi i parë është rreshti nga lart poshtë dhe i dyti është kolona nga e majta në të djathtë. Vargu përfshin kufirin e jashtëm prej muri, kështu që pjesa e brendshme e lundrueshme është `6×6`.
...
| Dosja | Rreshtat | `labels.json`? | Përdoreni për të |
|---|---:|---|---|
...
Çdo dosje përmban `observations.json`, një listë JSON të mostrave të përshkruara
...
Shkruani `predictions.json` në dosjen e punës të notebook-ut. Ai duhet të jetë një listë JSON
që përmban një veprim numër të plotë (`0`–`5`) për çdo rresht të
