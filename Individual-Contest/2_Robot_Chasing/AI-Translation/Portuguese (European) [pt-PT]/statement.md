...
Cada robô recebe uma instrução em inglês que descreve uma tarefa. O instantâneo pode ser obtido em qualquer momento enquanto o robô a executa. O seu objetivo é prever a próxima ação do robô.
...
| `image` | a sala, um array de inteiros `8×8×2` em que o canal 0 contém object_idx categórico (por exemplo, 1=vazio, 2=parede, 10=robô) e o canal 1 contém colour_idx categórico (0–5). |
| `direction` | a direção para a qual o robô está atualmente virado |
| `mission` | a instrução visível em linguagem natural |
| `carrying` | `null` ou `[object_idx, colour_idx]` para o objeto transportado |

As linhas são instantâneos independentes, dispostos por ordem aleatória. Não formam episódios e, no momento da avaliação, não está disponível qualquer observação ou ação anterior.

O `visualize_dataset.ipynb` fornecido permite-lhe inspecionar as observações disponíveis para o modelo em diferentes situações.
...
A imagem contém apenas os dois canais acima. A direção do robô é fornecida uma única vez, no campo `direction` de nível superior; não é duplicada dentro de `image`.
...
O campo `direction` indica a orientação atual através de: 0 = Cima (linha - 1), 1 = Baixo (linha + 1), 2 = Esquerda (coluna - 1), 3 = Direita (coluna + 1).

Uma ação de movimento começa por virar o robô para essa direção absoluta e, em seguida, tenta movê-lo uma célula. Uma parede ou um objeto pode bloquear o movimento, mas a direção muda na mesma. `pick up` e `drop` atuam exclusivamente sobre a célula-alvo adjacente definida pela direção (por exemplo, se direction=0, atua sobre (row - 1, col)).
...
| `dataset/test_public/` | 3,600 | incluído na cópia de desenvolvimento | executar e autoavaliar o seu pipeline |
...
No momento da classificação, `dataset/test_public/` é substituído de forma transparente por um conjunto oculto de
3,600 observações no mesmo formato, mas sem `labels.json`. A tabela classificativa
pública utiliza `test_leaderboard_a`; a classificação final utiliza
...
A pontuação é a **exatidão média por robô** numa escala de `0`–`100`. A exatidão é primeiro
...
5. Regresse à página do Concurso e clique em **Submeter**.
