...
로봇이 여섯 대 있습니다. 각 로봇은 그리드로 표현된 작은 방 안에서 작동합니다. 각 방에는 벽으로 둘러싸인 `6×6` 크기의 플레이 가능 영역이 있으므로, 전체 `image` 배열의 크기는 `8×8`입니다(플레이 가능 영역 + 벽).
...
같은 지시문이 여러 방식으로 표현될 수 있습니다. 테스트 세트에는 익숙한 표현, 색상, 물체 유형의 새로운 조합이 포함될 수 있습니다. 그러나 테스트 세트에서 사용되는 모든 단어, 표현 패턴, 색상, 물체 유형, 미션 유형은 학습 세트에도 등장합니다.
...
각 행은 무작위 순서로 배열된 독립적인 스냅샷입니다. 이들은 에피소드를 구성하지 않으며, 평가 시점에 이전 관측이나 행동은 제공되지 않습니다.
...
이동 행동은 먼저 로봇을 해당 절대 방향으로 돌린 다음, 한 칸 이동을 시도합니다. 벽이나 물체가 이동을 막을 수 있지만, 방향은 그래도 변경됩니다. `pick up`과 `drop`는 방향에 의해 정의되는 인접한 대상 칸에만 작용합니다(예: direction=0이면 (row - 1, col)에 작용합니다).
...
JSON 파일이 없거나 유효하지 않은 경우, 예측 개수가 잘못된 경우, 정수가 아닌 값이 있는 경우, 또는 `{0,1,2,3,4,5}` 범위를 벗어난 행동이 있는 경우에는 점수 없이 거부됩니다.
