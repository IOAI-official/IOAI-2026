...
Istnieje sześć robotów. Każdy robot działa w małym pomieszczeniu reprezentowanym przez siatkę. Każde pomieszczenie ma obszar gry `6×6` otoczony ścianami, więc pełna tablica `image` ma rozmiar `8×8` (obszar gry + ściany).

Każdy robot otrzymuje instrukcję w języku angielskim opisującą zadanie. Migawka może zostać wykonana w dowolnym momencie podczas wykonywania zadania przez robota. Twoim celem jest przewidzenie następnej akcji robota.
...
- **pójście do** obiektu, na przykład `"approach the red ball"`;
- **podniesienie** obiektu, na przykład `"grab the blue key"`;
...
| Pole | Znaczenie |
|---|---|
| `robot_id` | który z 6 robotów jest przedstawiony (`0`–`5`) |
| `image` | pomieszczenie, tablica liczb całkowitych `8×8×2`, w której kanał 0 zawiera kategoryczną wartość object_idx (np. 1=puste, 2=ściana, 10=robot), a kanał 1 zawiera kategoryczną wartość colour_idx (0–5). |
| `direction` | kierunek, w którym robot jest obecnie zwrócony |
| `mission` | widoczna instrukcja w języku naturalnym |
| `carrying` | `null` lub `[object_idx, colour_idx]` dla przenoszonego obiektu |

Wiersze są niezależnymi migawkami w losowej kolejności. Nie tworzą epizodów, a podczas ewaluacji nie jest dostępna żadna wcześniejsza obserwacja ani akcja.
...
Żetony mogą występować w pomieszczeniu, ale nigdy nie są wymieniane w misjach.

Identyfikatory kolorów to `0` czerwony, `1` zielony, `2` niebieski, `3` fioletowy, `4` żółty oraz `5` szary. Kanał koloru nie ma znaczenia dla pustych pól i ścian.

Obraz ma wyłącznie dwa powyższe kanały. Kierunek robota jest podany jednokrotnie, w polu najwyższego poziomu `direction`; nie jest powielony wewnątrz `image`.
...
| `dataset/test_public/` | 3,600 | dołączone w kopii deweloperskiej | uruchamianie potoku i samodzielna ocena jego wyniku |

Każdy folder zawiera `observations.json`, czyli listę JSON opisanych
powyżej próbek. `labels.json` jest odpowiadającą jej listą JSON akcji (`0`–`5`).

Zbiór treningowy zawiera dokładnie 10,000 wierszy na robota oraz 20,000 wierszy z każdej
rodziny zadań. Publiczny zbiór testowy zawiera 600 wierszy na robota. Jeśli potrzebujesz tablicy, opakuj `image` za pomocą
`numpy.asarray(...)`.
...
Odczytuj etykiety wyłącznie z `dataset/train/`.
...
albo akcja spoza `{0,1,2,3,4,5}` zostaną odrzucone bez przyznania wyniku.

## Ocenianie

Wynikiem jest **średnia dokładność dla poszczególnych robotów** na skali `0`–`100`. Dokładność jest najpierw
...
4. W zakładce Git w JupyterLab dodaj do obszaru przejściowego i zatwierdź `solution.ipynb`, a następnie go wypchnij.
5. Wróć na stronę konkursu i kliknij **Prześlij**.
