...
Kemi gjashtë robotë. Çdo robot vepron në një dhomë të vogël të përfaqësuar nga një tabel. Çdo dhomë ka një zonë të luajtshme `6×6` të rrethuar me mure, kështu që vargu i plotë `image` ka madhësi `8×8` (zona e luajtshme + muret).

Çdo robot merr një udhëzim në anglisht që përshkruan një detyrë. Pamja e çastit mund të merret në cilëndo pikë ndërsa roboti po e kryen detyrën. Synimi juaj është të parashikoni veprimin e ardhshëm të robotit.

Robotët nuk ndjekin gjithmonë shtegun më të shkurtër. Roboti 0 mund të sillet ndryshe nga Roboti 1, por çdo robot ndjek modelin e vet të qëndrueshëm. Përdorni shembujt e trajnimit, të cilët përfshijnë veprimet e sakta të ardhshme, për t'i mësuar këto modele.
...
I njëjti udhëzim mund të shkruhet në disa mënyra. Bashkësia e testimit mund të përmbajë kombinime të reja frazash, ngjyrash dhe llojesh objektesh të njohura. Megjithatë, çdo fjalë, model fraze, ngjyrë, lloj objekti dhe lloj misioni i përdorur në bashkësinë e testimit shfaqet gjithashtu në bashkësinë e trajnimit.
...
| `image` | dhoma, një matric me numra  të plotë `8×8×2` ku kanali 0 përmban object_idx me kategori(p.sh., 1=bosh, 2=mur, 10=robot) dhe kanali 1 përmban colour_idx me kategori (0–5). 
| `direction` | drejtimi nga i cili është kthyer aktualisht roboti |
| `mission` | udhëzimi i dukshëm në gjuhë natyrore |
| `carrying` | `null` ose `[object_idx, colour_idx]` për objektin e mbajtur |

Rreshtat përmbajnë pamje të pavarura të çastit , me rend të rastësishëm. Ato nuk formojnë episode dhe gjatë vlerësimit nuk është i disponueshëm asnjë vëzhgim ose veprim i mëparshëm.
...
`image[row][column] = [object_idx, colour_idx]`. Indeksi i parë është rreshti nga lart poshtë dhe i dyti është kolona nga e majta në të djathtë. Matrica përfshin kufirin e jashtëm prej muri, kështu që pjesa e brendshme është `6×6`.
...
Imazhi ka vetëm dy kanalet e mësipërme. Drejtimi i robotit jepet një herë,  në nivelit e sipërme  `direction`të fushës ; ai nuk duplikohet brenda `image`.
...
Ju jepen dy folder-a:

| Folder Rreshtat | `labels.json`? | Përdoreni për të |
|---|---:|---|---|
...
Shkruani `predictions.json` në folder-in e punës. Ai duhet të jetë një listë JSON
që përmban një veprim me një numër të plotë (`0`–`5`) për çdo rresht të
`dataset/test_public/observations.json`, në të njëjtin rend. Për një bashkësi hipotetike testimi që përmban gjashtë mostra, një dalje e vlefshme do të ishte:

```json
[0, 3, 2, 2, 5, 4]
```

Një skedar JSON që mungon ose është i pavlefshëm, një numër i gabuar parashikimesh, një vlerë jo e plotë,
...
llogaritet në mënyrë të pavarur për çdo robot, pastaj mesatarizohet për të gjashtë robotët. Prandaj, çdo
...
3. Përmirësojeni modelin nëse dëshironi; baseline-i i dhënë demonstron vetëm
   formatin e kërkuar të hyrjes dhe daljes.
4. Në skedën Git të JupyterLab, bëni stage dhe commit `solution.ipynb`, pastaj bëni push.
5. Kthehuni në faqen e konkursit dhe klikoni **Dorëzo**.

Dorëzoni saktësisht një skedar me emrin `solution.ipynb`.
