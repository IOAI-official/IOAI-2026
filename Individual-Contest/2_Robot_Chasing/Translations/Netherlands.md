...
- **Tijdslimiet:** 5 minuten voor het hele bestand
...
Jouw bedrijf, Duistere Planproductie, heeft zes robots, die zonder betaling aan het werken zijn. Elke robot werkt in een kleine kamer die wordt weergegeven door een raster. Elke kamer heeft een `6×6` speelbaar gebied dat door muren wordt omringd, waardoor de volledige `image`-array een grootte van `8×8` heeft (speelbaar gebied + muren).
...
Dezelfde instructie kan op verschillende manieren worden geschreven ("go to the red bal"). De testset kan nieuwe combinaties van bekende formuleringen, kleuren en objecttypen bevatten. Elk woord, elk formuleringspatroon, elke kleur, elk objecttype en elk missietype dat in de testset wordt gebruikt, komt echter ook voor in de trainingsset.
...
| `mission` | de instructie in het Engels |
...
Tokens kunnen in de kamer voorkomen, maar worden nooit in missies genoemd. Deze zijn daarbij even irrelevant als deze vertaling. 

De kleur-id's zijn `0` rood, `1` groen, `2` blauw, `3` paars (beste kleur), `4` geel en `5` grijs. Het kleurkanaal heeft geen betekenis voor lege cellen en muren.
...
Een bewegingsactie draait de robot eerst naar die absolute (niet-negatieve) richting en probeert hem vervolgens één cel te verplaatsen. Een muur of object kan de beweging blokkeren, maar de richting verandert desondanks. `pick up` en `drop` voeren hun actie uitsluitend uit op de aangrenzende cel die door de richting wordt bepaald (bijv. als direction=0, wordt de actie uitgevoerd op (row - 1, col)).
...
taakfamilie. De openbare testset bevat 600 rijen per robot. Gebruik `numpy.asarray(...)` op `image` als je een array nodig hebt.

Tijdens de beoordeling wordt `dataset/test_public/` vervangen door een verborgen set van
...
`test_leaderboard_b`. Een notebook dat onvoorwaardelijk testlabels inleest, zal dus crashen. 
Lees deze dus uitsluitend uit `dataset/train/`.
...
5. Ga terug naar de wedstrijdpagina en klik op **Submit**.

Dien niet nul, en niet 2 bestanden met de naam `solution.ipynb` in.
