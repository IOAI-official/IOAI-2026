...
- **colocar un objeto junto a otro**, por ejemplo
  `"place the red box beside the green ball"` (coloca la caja roja al lado de la bola verde).
...
`image[fila][columna] = [objeto_idx, color_idx]`. El primer índice es la fila, de arriba abajo, y el segundo es la columna, de izquierda a derecha. El array incluye el borde exterior de paredes, por lo que el interior transitable es `6×6`.
...
familia de tareas. El conjunto de prueba público contiene 600 filas por robot. Cubra (wrap) `image` con
