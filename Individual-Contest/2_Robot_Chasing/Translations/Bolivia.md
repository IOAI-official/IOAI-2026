...
| Campo | Significado |
|---|---|
| `robot_id` | cuál de los 6 robots es este (`0`–`5`) |
...
Las filas son 'snapshots' independientes en orden aleatorio. No forman episodios, y durante la evaluación no se dispone de ninguna observación ni acción anterior.
...
`image[row][column] = [object_idx, colour_idx]`. El primer índice es la fila, de arriba hacia abajo, y el segundo es la columna, de izquierda a derecha. El array incluye el borde exterior de paredes, por lo que el interior transitable es `6×6`.
...
| Carpeta | Filas | ¿`labels.json`? | Utilizala para |
|---|---:|---|---|
...
familia de tareas. El conjunto de prueba público contiene 600 filas por robot. Encierra `image` con
`numpy.asarray(...)` si necesitas un array.
...
Genera `predictions.json` en el directorio de trabajo del notebook. Debe ser una lista
...
La puntuación es la **accuracy media por robot** en una escala de `0`–`100`. Primero se calcula la accuracy
...
2. Confirma que escribe `predictions.json` con 3,600 predicciones para el conjunto de
   prueba público.
3. Mejora el modelo si deseas; el baseline proporcionado solo muestra el
   formato requerido de entrada y salida.
4. En la pestaña Git de JupyterLab, prepara y confirma `solution.ipynb` y, a continuación, haz push.
5. Vuelve a la página del concurso y haz click en **Enviar**.

Envia exactamente un archivo llamado `solution.ipynb`.
