...
Chaque robot reçoit une instruction en anglais décrivant une tâche. Le snapshot peut être pris à n'importe quel moment pendant que le robot l'exécute. Votre objectif est de prédire la prochaine action du robot.
...
`image[row][column] = [object_idx, colour_idx]`. Le premier ID correspond à la ligne, de haut en bas, et le second à la colonne, de gauche à droite. Le tableau comprend la bordure extérieure constituée de murs, de sorte que l'intérieur navigable est `6×6`.
...
Les ID des couleurs sont `0` rouge, `1` vert, `2` bleu, `3` violet, `4` jaune et `5` gris. Le canal des couleurs n'a aucune signification pour les cases vides et les murs.
...
Le score est la **precision moyenne par robot** sur une échelle de `0`–`100`. La precision est d'abord
calculée indépendamment pour chaque robot, puis on calcule la moyenne sur les six robots. Chaque
...
4. Dans l'onglet Git de JupyterLab, ajoutez et commitez `solution.ipynb`, puis faites le Push.
5. Retournez à la page du concours et cliquez sur **Submit**.
