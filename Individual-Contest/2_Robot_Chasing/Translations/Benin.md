...
Chaque robot reçoit une instruction en anglais décrivant une tâche. Une capture d'écran (ou _snapshot_) peut être pris à n'importe quel moment pendant que le robot exécute la tâche. Votre objectif est de prédire la prochaine action du robot.
...
- **aller jusqu'à (_go to_)** un objet, par exemple `"approach the red ball"` ;
- **ramasser (_pick up_)** un objet, par exemple `"grab the blue key"` ;
- **placer un objet à côté d'un autre (_put one object next to another_)**, par exemple
  `"place the red box beside the green ball"`.

Une même instruction peut être formulée de plusieurs manières. La base de test peut contenir de nouvelles combinaisons de formulations, de couleurs et de types d'objets connus. Cependant, chaque mot, modèle de formulation, couleur, type d'objet et type de mission utilisé dans la base de test apparaît également dans la base d'entraînement.
...
Les lignes sont des captures d'écran (ou _snapshot_) indépendants, présentés dans un ordre aléatoire. Elles ne constituent pas des épisodes, et aucune observation ou action antérieure n'est disponible au moment de l'évaluation.
...
Le champ `direction` indique l'orientation actuelle selon la convention suivante : 0 = Haut (ligne - 1) ou Up (row - 1), 1 = Bas (ligne + 1) ou Down (row + 1), 2 = Gauche (colonne - 1) ou Left (col - 1), 3 = Droite (colonne + 1) ou Right (col + 1).

Une action de déplacement oriente d'abord le robot dans cette direction absolue, puis tente de le déplacer d'une case. Un mur ou un objet peut bloquer le déplacement, mais la direction change tout de même. `pick up` et `drop` agissent exclusivement sur la case cible adjacente définie par la direction (par exemple, si direction=0, l'action porte sur (ligne - 1, colonne) ou (row - 1, col)).

## Dataset

Vous recevez deux dossiers :

| Dossier | Lignes | `labels.json` ? | Utilisez le pour |
|---|---:|---|---|
| `dataset/train/` | 60,000 | inclus | entraîner votre modèle |
| `dataset/test_public/` | 3,600 | inclus mais disponible apres soumission| exécuter et évaluer vous-même votre pipeline |
...
La base d'entraînement contient exactement 10,000 lignes par robot et 20,000 lignes de chaque
famille de tâches. La base de test public contient 600 lignes par robot. Encapsulez `image` avec
`numpy.asarray(...)` si vous avez besoin d'un tableau.

Au moment de l'evaluation, `dataset/test_public/` est remplacé de manière transparente par un ensemble caché de
...
NB : _Pas de modeles pré entrainés disponibles pour ce challenge.
...
`dataset/test_public/observations.json`, dans le même ordre. Pour une base de test hypothétique contenant six échantillons, une sortie valide serait :
...
Le score est l'**exactitude (_accuracy_) moyenne par robot** sur une échelle de `0`–`100`. L'exactitude (_accuracy_) est d'abord calculée indépendamment pour chaque robot, puis moyennée sur les six robots. Chaque robot a donc le même poids.

## Comment soumettre

1. Ouvrez le fichier `solution.ipynb` et exécutez toutes les cellules.
2. Vérifiez qu'il écrit `predictions.json` avec 3,600 prédictions pour la base de
   test public.
...
5. Retournez à la page du _challenge_ et cliquez sur **Soumettre**.
