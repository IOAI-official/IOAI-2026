...
Su amigo le propone jugar a un juego de adivinanzas.
Él, como juez, elige una palabra oculta de un vocabulario fijo, y usted debe encontrarla en 30 turnos como máximo.
En cada turno, el juez compara dos palabras e indica cuál tiene un significado más cercano al de la palabra oculta. Cada partida comienza con
...
el sistema crea una instancia nueva de PublicEmbeddingPlayer al comienzo de cada partida.
...
Las opciones de `verdict` son `first`, que significa que la palabra 1 (word1) está más cerca; `second`, que significa que la palabra 2 (word2) está más cerca; o
`same`, que significa que ambas palabras están igual de cerca de la palabra oculta. 

`winner_word` es la palabra que se conserva para la siguiente comparación. Con un veredicto `same`, se escoge la primera palabra.
...
[Referencia del modelo Embedding  Qwen](https://yastatic.net/s3/contest/ioai/3/01_Qwen_Qwen3-Embedding-0.6B_MODEL_CARD.html)
models/BAAI/bge-m3
[Referencia del modelo bge-m3](https://yastatic.net/s3/contest/ioai/3/02_BAAI_bge-m3_MODEL_CARD.html)
```

Ambos deben cargarse desde su ruta local; un identificador de Hugging Face como `"BAAI/bge-m3"` intentara activar una descarga y falla, porque la evaluación se realiza sin conexión. Lea en cada directorio el archivo  `example.py`  que muestra la llamada sin conexión.

Bibliotecas disponibles: `numpy`, `torch`, `sentence-transformers`. No dispone de Internet, ni
descargas ni otros paquetes.
...
Tiene un límite de 10 minutos que abarca el inicio, la preparación y las 120
