...
- `dataset/public_embeddings.npy` — `float32`, forması (shape) `(1602, 2560)`. `i` sətiri
  lüğətdəki `i` sözünə uyğun gəlir. Bunlar *ictimai* (public) embeddinglərdir;
  hakim fərqli, məxfi təsvirdən (private representation) istifadə edir.
...
Tapşırıqla birlikdə öncədən təlimatlandırılmış (pretrained) iki embedding modeli təqdim olunur və istifadə edilə bilər:
...
2. İstəyə bağlı olaraq, onu lokal şəkildə yoxlayın: `python local_test.py solution.ipynb --limit 5`.
   Lokal hakim *ictimai* (public) embedding istifadə edir, buna görə də onun xalı yalnız orientir xarakteri daşıyır.
