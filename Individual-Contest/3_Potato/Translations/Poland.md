...
- **Środowisko:** 1 GPU (≈16 GB VRAM), bez internetu
...
ustalonej pary `lamp vs potato` (lampa czy ziemniak), ponieważ są to dwie z ulubionych rzeczy Twojego znajomego. Następnie Twój program
...
Możesz zmienić klasę PublicEmbeddingPlayer. Twój program jest inicjalizowany raz i rozgrywa każdą grę w jednym uruchomieniu;
...
```text
Ukryte słowo: shovel          Początek zawsze ten sam: lamp vs potato
...
`winner_word` to słowo, które zostaje zachowane do następnego porównania. W przypadku werdyktu `same` pierwsze słowo zostaje zachowane.
...
- `dataset/public_embeddings.npy` — `float32`, kształt `(1602, 2560)`. Wiersz `i`
  ...
  sędzia używa innych, prywatnych reprezentacji.
...
| `test_leaderboard_a` | 120 | ukryte | ranking "na żywo" |
| `test_leaderboard_b` | 120 | ukryte | ranking końcowy |

Nie ma podziału `train` — nic nie jest trenowane (ang. fitted) na podstawie oetykietowanych wierszy.

### Dostarczone modele

Wraz z zadaniem dostarczane są dwa pretrenowane modele reprezentacji wektorowych (embeddingów), których można użyć:
...
Oba muszą zostać wczytane ze swojej lokalnej ścieżki; użycie Hugging Face hub id, takiego jak
`"BAAI/bge-m3"`, powoduje próbę pobrania modelu z internetu i kończy się niepowodzeniem, ponieważ ocenianie odbywa się offline. Każdy
katalog zawiera działający (ang. runnable) plik `example.py` demonstrujący wykonanie offline.
...
Limit 10 minut jest obejmuje uruchomienie, przygotowanie oraz wszystkie 120
...
5. _Dodaj_ `solution.ipynb` _do obszaru przejściowego_ (ang. stage) (ikona **+** obok niego).
6. Wprowadź opis commita i kliknij Commit.
7. Kliknij ikonę chmury ze strzałką w górę, aby wykonać push.
8. Wróć na tę stronę konkursu i kliknij Submit, używając opisu commita zgodnego z wcześniej podanym.
