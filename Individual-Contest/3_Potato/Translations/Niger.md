...
la paire fixe `lamp vs potato`, car ce sont deux des choses préférées de votre ami. Votre programme
...
compte de la casse. Chaque mot que vous proposez doit etre dans `dataset/vocabulary.json`.

Il ya un exemple complet avec le protocole et le chargement des données qui se trouve dans `solution.ipynb`. 
...
`"BAAI/bge-m3"` déclenche un téléchargement et échoue, comme l'évaluation est effectuée hors ligne. Chaque
...
2. Facultativement, vérifiez-le localement : `python local_test.py solution.ipynb --limit 5`.
   Le juge local utilise les plongements *publics*, son score n'est donc
   qu'indicatif.
...
8. Revenez sur la page du concours et cliquez sur Submit, en utilisant le même message de commit que celui que vous avez fourni.

Soumettez exactement un seul fichier, nommé `solution.ipynb`, couvrant toute préparation et toute inférence nécessaires.
