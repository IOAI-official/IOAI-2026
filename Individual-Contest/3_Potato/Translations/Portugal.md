...
Ele, como juiz, escolhe uma palavra oculta de um vocabulário fixo, e o seu objetivo será encontrar a palavra secreta em, no máximo, 30 jogadas.
...
o seguinte par fixo `lamp vs potato`, porque estas são duas das coisas preferidas do seu amigo. Em seguida, o seu programa
propõe uma nova palavra. A palavra vencedora da comparação é mantida
e comparada com a sua palavra seguinte. 
...
- `dataset/vocabulary.json` — 1602 palavras únicas em minúsculas. A palavra oculta é sempre
  uma destas.
- `dataset/public_embeddings.npy` — `float32`, de dimensão `(1602, 2560)`. Onde a linha `i`
...
| `test_leaderboard_a` | 120 | ocultas | classificação na leaderboard pública |
| `test_leaderboard_b` | 120 | ocultas | classificação final |

Não existe nenhuma partição `train` — uma vez que nada é ajustado a partir de linhas rotuladas.
...
diretório contém um `example.py` executável que mostra como fazer uma chamada offline.

As seguintes bibliotecas estão disponíveis: `numpy`, `torch`, `sentence-transformers`. Sem internet, sem
downloads e sem outros pacotes.

## Saída

Nenhuma. Esta é uma tarefa interativa: a sua solução não escreve nenhum ficheiro de resposta; apenas comunica com
...
2. Opcionalmente, pode verificar localmente com: `python local_test.py solution.ipynb --limit 5`.
...
5. Coloque `solution.ipynb` em staging (o símbolo **+** junto ao ficheiro).
...
Submeta exatamente um ficheiro, denominado `solution.ipynb`, que abranja toda a preparação e inferência necessária.
