...
每回合，裁判會比較兩個詞，並回報哪一個在語意上更接近
隱藏詞。每場遊戲都從
固定詞對 `lamp vs potato` 開始，因為它們是你朋友最喜歡的兩樣東西。接著，你的程式
提出一個新詞。比較中的勝者會被保留，
並與你的下一個提案進行比較。
當你提出的詞與隱藏詞完全相同時，你便立即贏得該場遊戲。比對
不區分大小寫。你提出的每個詞都必須位於 `dataset/vocabulary.json` 中。

`solution.ipynb` 中提供了包含通訊協定與資料載入的完整範例。
你可以修改 PublicEmbeddingPlayer 類別。你的程式只會初始化一次，並在單次執行中進行所有遊戲；
通訊協定會在每場遊戲開始時建立一個新的 PublicEmbeddingPlayer。
...
- `dataset/vocabulary.json` — 1602 個不重複的小寫詞。隱藏詞必定
  是其中之一。
...
可用的函式庫：`numpy`、`torch`、`sentence-transformers`。無網際網路、不可
下載，也沒有其他套件。
