...
pair bi ñu saxal `lamp vs potato`, ndaxte ñoom ñaar ñoo bokk ci mbir yi sa xarit gën a bëgg. Sa program bi dina
joxe benn baat bu bees. Baat bi raw ci méngale bi, dees koy denc
te méngale ko ak sa proposal bi ci topp. 
Danga daan po bi saa si nga joxee baat bi làqu ci anam wu dëppoo. Dëppale bi
case-insensitive la. Bépp baat boo joxe war na nekk ci `dataset/vocabulary.json`.

Am na misaal bu mat sëkk ci `solution.ipynb`, ànd ak protocol ak data loading. 
Mën nga soppi class PublicEmbeddingPlayer. Dees na initialise sa program bi benn yoon, mu fo po yépp ci benn run;
protocol bi dafay sos PublicEmbeddingPlayer bu bees ci njàlbéenug bépp po.

## Àttekat bi

Sa program bi dafay yónnee Àttekat bi benn JSON object, Àttekat bi tontu ak benn JSON object. 

Misaal bu ñu liggéeyal, te ñu wone baat bi làqu ngir rekk leeral protocol bi:
...
Split yépp dañuy bokk yii:

- `dataset/vocabulary.json` — 1602 baat yu bokkul te ñépp lowercase lañu. Baat bi làqu dafay nekk saa su nekk
  benn ci yii.
- `dataset/public_embeddings.npy` — `float32`, shape `(1602, 2560)`. Row `i`
  dafay méngoo ak baat `i` ci vocabulary bi. Yii ay embedding *public* lañu;
  àttekat bi dafay jëfandikoo representation bu wuute te private.

Split yi ay mbooloom baat yu làqu lañu:

| Split | Baat yi | Tontu yi | Jëfandikoo ko ngir |
|---|---|---|---|
| `test_public` | 120 | ✅ `dataset/test_public.json` | doxal sa solution te sañ-sañal sa bopp |
| `test_leaderboard_a` | 120 | làqu | leaderboard buy dox |
| `test_leaderboard_b` | 120 | làqu | classement bu mujj |

Amul split `train` — dara du def fit ci row yu am label.
...
Dees na leen ñaar yépp load ci seen local path; Hugging Face hub id bu mel ni
`"BAAI/bge-m3"` dafay déclencher download te du antu, ndax àtte bi offline la. Bépp
directory am na `example.py` bu mën a dox, biy wone offline call bi.

Library yi am: `numpy`, `torch`, `sentence-transformers`. Internet amul, download amul,
te package bu ñeneen amul.

## Output

Dara. Liggéey bu interactive la: sa solution du bind answer file; dafay jokkoo ak
...
Diggante 10 simili bi benn budget la buy mboole start-up, preparation ak po yépp 120
ci test set bi. 

## Naka lañuy submit

1. Ubbi `solution.ipynb`, soppi `PublicEmbeddingPlayer`, te doxal cells yépp ngir wóor ne dafay dox.
...
5. Def Stage `solution.ipynb` (icon **+** bi ci wetam).
...
Submit benn file rekk, turam di `solution.ipynb`, buy mboole bépp preparation ak inference bu war.
