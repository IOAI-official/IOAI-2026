...
- `dataset/public_embeddings.npy` — `float32`、shape は `(1602, 2560)`。行 `i` は、語彙内の単語 `i` に対応します。これらは*公開* embedding です。ジャッジは別の非公開表現を使用します。
...
必要な準備と inference のすべてを含む、`solution.ipynb` という名前のファイルを正確に 1 つ提出してください。
