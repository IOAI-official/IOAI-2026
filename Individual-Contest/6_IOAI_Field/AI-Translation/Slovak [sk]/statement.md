...
- **Východiskové skóre**: 31.2187
- **Skóre vedeckého výboru:** 63.53
...
- triedy trénovacieho modelu uloženej ako `custom_model.py`. Tento model by mal dediť z triedy `torch.nn.Module` a používať iba importy `torch`. Mal by obsahovať triedu `CustomModel` použitú v notebooku `solution.ipynb`. 
...
Pre každú oblasť $R$ (prvé písmeno `I`, `O`, `A`, `Background`) vyhodnocujeme model na $N_R = 512$ testovacích bodoch $(x_i, y_i)$ so skutočnými hodnotami $v_i$ a predikciami $\hat{v}_i$. Ako hlavnú metriku používame normalizovanú strednú absolútnu chybu (MAE). MAE je definovaná ako:
...
5. Vráťte sa na stránku súťaže a kliknite na **Submit**. Komentár k odovzdaniu by mal byť rovnaký ako komentár z predchádzajúceho kroku.
