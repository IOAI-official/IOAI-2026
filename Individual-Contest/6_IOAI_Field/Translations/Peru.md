...
El número de parámetros se mide mediante `sum(p.numel() for p in model.parameters())`. Se espera que el modelo también funcione en un modo estocástico y tambien con el `nn.Dropout` de PyTorch como parte del modelo.
...
5. Regrese a la página del Contest y haga clic en **Submit**. El comentario de la entrega debe ser el mismo que el comentario del paso anterior.
