...
- Os valores dentro da letra `A` são sempre -1
- Os valores no interior da última letra `I` devem ser valores aleatórios no intervalo $[-2026,2026]$, mesmo quando avaliados duas vezes no mesmo ponto
- Fora das letras, o valor é sempre zero

A função tem parâmetros ocultos $\overline{W}$, que afetam a escala e a inclinação das letras, juntamente com o intervalo de valores no interior da primeira letra `I`. No entanto, as letras não se vão intersetar. Eis alguns exemplos ilustrativos do aspeto do campo IOAI com diferentes $\overline{W}$:
