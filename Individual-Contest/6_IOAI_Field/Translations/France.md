...
La configuration de test est cachée, mais elle est de nature similaire. Votre tâche consiste à ajuster votre modèle sur le générateur fourni en utilisant autant de données que vous le souhaitez. Vos distributions « train » et « test » sont générées par le même générateur — vous ne savez simplement pas sur quels points $(x_i, y_i)$ vous serez évalué.
...
Pour chaque région $R$ (première lettre `I`, `O`, `A`, `Background`), nous évaluons le modèle sur $N_R = 512$ points de test $(x_i, y_i)$, avec les valeurs réelles $v_i$ et les prédictions $\hat{v}_i$. Nous utilisons l’erreur absolue moyenne normalisée (MAE) comme métrique principale. La MAE est définie comme suit :
...
3. Sinon, nous calculons l’écart-type $\sigma_j$ des $K$ sorties et le convertissons en score :
...
4. Dans l’onglet Git de JupyterLab, stage (ajoutez à l’index), commentez et commit `solution.ipynb` et `custom_model.py`, puis push les.
